...
def check(name: str, method: str, path: str, body: dict | None = None,
           expected_status: int = 200, api_key: str | None = API_KEY,
           validate: callable = None) -> bool:
    ...
    try:
        ...
        status = resp.status
        resp_body = json.loads(resp.read().decode()) if resp.headers.get("Content-Type", "").startswith("application/json") else resp.read().decode()
    ...
    if passed and validate:
        try:
            ...
            passed = False
            CHECKS.append({"name": name, "status": "FAIL", "error": f"Validation: {e}", "http_status": status, "latency_ms": round(elapsed_ms, 1)})
            return False
...
def main() -> int:
    ...
    # Health endpoint
    check("GET /health", "GET", "/health", api_key=None,
          validate=lambda r: assert_field(r, "status", "ok"))
    ...
    # Runtime prediction
    check("POST /v1/runtime/predict", "POST", "/v1/runtime/predict",
    ...
    check("POST /v1/runtime/predict (minimal)", "POST", "/v1/runtime/predict",
          body={"requested_cpus": 1})

    # Resource-fit prediction
    check("POST /v1/resource-fit/predict", "POST", "/v1/resource-fit/predict",
    ...
    # Metrics endpoint
    check("GET /metrics", "GET", "/metrics", api_key=None,
          validate=lambda r: "hpcopt" in str(r) if r else True)

    # Admin log level (requires admin key)
    check("POST /v1/admin/log-level (admin)", "POST", "/v1/admin/log-level",
          body={"level": "INFO"}, api_key=ADMIN_KEY)

    # Admin log level (non-admin key should 403)
    check("POST /v1/admin/log-level (non-admin)", "POST", "/v1/admin/log-level",
          body={"level": "DEBUG"}, expected_status=403, api_key=API_KEY)

    # Auth enforcement (no key should 401)
    check("POST /predict (no key → 401)", "POST", "/v1/runtime/predict",
          body={"requested_cpus": 1}, expected_status=401, api_key=None)

    # Validation (invalid input → 422)
    check("POST /predict (invalid → 422)", "POST", "/v1/runtime/predict",
          body={"requested_cpus": 0}, expected_status=422)

    # Recommendation not found → 404
    check("GET /recommendations/nonexistent → 404", "GET",
          "/v1/recommendations/nonexistent-run-id", expected_status=404)
    ...
    for c in CHECKS:
        ...
        extra = f" [expected={c.get('expected_status')}, got={c.get('http_status')}]" if c.get("status") == "FAIL" and "http_status" in c else ""
